...
object POST {
  ...
  def apply(uri: String, headers: Map[String, String] = Map(), request: Array[Byte] = Array()): Option[Response] = {
    try {
      ...
      def copy(in: InputStream, out: OutputStream): Unit = {
        ...
        var bytes_read = -1
        while ( {
          bytes_read = in.read(buffer, 0, buffer.length); bytes_read != -1
        }) {
      ...
      try {
        Some(Response(response_code, response_headers, toByteArray(response_stream)))
      } finally {
